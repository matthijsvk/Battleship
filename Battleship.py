...
#######LET'S PLAY!########
    ...
    while turn< maxturns and nb_ships_left>0:
        ...
        guess_x = int(raw_input("Enter X-coordinates to hit:"))-1 #-1 to use the 'logical' number as input (so '1' instead of '0')
        guess_y = board_height- int(raw_input("Enter Y-coordinates to hit:")) #board_height- to have a Y-axis pointing upward
        print"\n"
        ...
        if (occupied_by_other_ship([guess_y,guess_x],ships_left)): #first y, then x because the height is the y-th sublist of 'board'
            ...
            if (guess_y < 0 or guess_y >= board_height) or (guess_x < 0 or guess_x >= board_width): #if guess is out of bounds
              print "Oops, that's not even in the ocean.\n"
            elif(board[guess_y][guess_x] == "X" or board[guess_y][guess_x]== "H" or board[guess_y][guess_x]== "S"): #if already guessed, hit or sunk
...
##                print ship
    ...
    yn=raw_input("Do you want to play some more? \n")
    yn=yn.lower()
